...
size_t htx_xfer(struct htx *dst, struct htx *src, size_t count, unsigned int flags)
{
  ...
  stop:
	if (!(flags & HTX_XFER_PARTIAL_HDRS_COPY)) {
		...
		struct htx_blk *dstblk2;
		enum htx_blk_type type = HTX_BLK_UNUSED;

		dstblk2 = htx_get_tail_blk(dst);
		if (dstblk2)
			type = htx_get_blk_type(dstblk2);
	...
	if (!(flags & HTX_XFER_KEEP_SRC_BLKS)) {
		...
		if (!blk)
			 ...
			 */
			if (dstblk && blk2 && htx_get_blk_type(blk2) == HTX_BLK_DATA) {
				htx_cut_data_blk(src, blk2, htx_get_blksz(dstblk));
			}
		}
	}
