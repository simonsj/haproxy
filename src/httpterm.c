...
int hstream_buf_available(void *target)
{
	...
	if ((hs->state & HS_ST_IN_ALLOC) && b_alloc(&hs->req, DB_CHANNEL)) {
		hs->state &= ~HS_ST_IN_ALLOC;
		tasklet_wakeup(hs->sc->wait_event.tasklet);
		return 1;
	}

	if ((hs->state & HS_ST_OUT_ALLOC) && b_alloc(&hs->res, DB_CHANNEL)) {
		hs->state &= ~HS_ST_OUT_ALLOC;
		tasklet_wakeup(hs->sc->wait_event.tasklet);
		return 1;
	}
...
static int hstream_recv(struct connection *conn, struct hstream *hs)
{
	...
	fprintf(stderr, "%s(1): %d %d\n", __func__, !!(conn->flags & CO_FL_ERROR), sc_ep_test(hs->sc, SE_FL_ERROR));
  ...
  end_recv:
	...
	fprintf(stderr, "%s(2): req_body=%llu cur_read=%zu %d %d is_empty? %d data=%zu"
	        " sc_ep_test(hs->sc, SE_FL_WANT_ROOM...)? %d %d %d\n", 
	        __func__, hs->req_body, cur_read,
	        !!(conn->flags & CO_FL_ERROR),
	        sc_ep_test(hs->sc, SE_FL_ERROR),
	        is_empty,
	        b_data(&hs->req),
	        sc_ep_test(hs->sc, SE_FL_WANT_ROOM ),
	        sc_ep_test(hs->sc, SE_FL_ERROR),
	        sc_ep_test(hs->sc, SE_FL_EOS));
	...
	else if ((hs->req_body || !cur_read) && !sc_ep_test(hs->sc, SE_FL_WANT_ROOM | SE_FL_ERROR | SE_FL_EOS)) {
			...
			if (next > arg + 2) {
				...
				switch (*arg) {
					...
					case 'k':
						...
						break;
					case 'b':
						hs->req_bodylen = result;
						break;
					case 'B':
						hs->req_maxbody = result;
						break;
					case 'S':
						hs->req_nosplice = !result;
						break;
					case 'R':
...
struct hstream *hstream_new(struct session *sess, struct stconn *sc, struct buffer *input)
{
